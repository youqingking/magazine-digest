...
const outDir = path.join(repoRoot, "output", "stage-h0_5-h5");
const reportPath = path.join(outDir, "device-diagnostics-report.json");
const wrapperReportPath = path.join(outDir, "device-diagnostics-wrapper.json");
const logPath = path.join(outDir, "hbuilderx-web.log");
const buildDir = path.join(mobileProjectRoot, "unpackage", "dist", "build", "web");
const defaultUsername = "qqqq";
const defaultPassword = "1111qqqq";
...
function resolveProjectName(cliPath) {
  const output = execFileSync(cliPath, ["project", "list"], {
    cwd: repoRoot,
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"],
    maxBuffer: 16 * 1024 * 1024
  ...
  const projectLine = output
    .split(/\r?\n/)
    .map((line) => line.trim())
    .find((line) => /^\d+\s+-\s+mobile(?:\(|\b)/.test(line));

  if (!projectLine) {
    throw new Error("HBUILDERX_PROJECT_MOBILE_MISSING");
  }

  return "mobile";
}

function uploadCloudFunction(cliPath, projectName, functionName) {
  return execFileSync(
    cliPath,
    ["cloud", "functions", "--upload", "cloudfunction", "--prj", projectName, "--provider", "aliyun", "--name", functionName],
    {
      cwd: repoRoot,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
  ...
  return new Promise((resolve) => {
    const request = http.get(url, (response) => {
      ...
      response.on("end", () => {
        ...
        resolve(response.statusCode >= 200 && response.statusCode < 500 && /<html|<!doctype html/i.test(body));
      });
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    const args =
      process.platform === "win32"
        ? ["/d", "/s", "/c", "npx playwright test tests/smoke/h0_5-device-diagnostics.spec.js --reporter=line"]
        : ["playwright", "test", "tests/smoke/h0_5-device-diagnostics.spec.js", "--reporter=line"];
...
async function main() {
  ...
  openProject(cliPath);
  const projectName = resolveProjectName(cliPath);
  const uploadOutput = [
    uploadCloudFunction(cliPath, projectName, "device-sync-co"),
    uploadCloudFunction(cliPath, projectName, "h0_5-web-auth-smoke"),
    execFileSync(
      cliPath,
      ["cloud", "functions", "--upload", "common", "--prj", projectName, "--provider", "aliyun", "--name", "uni-config-center", "--unimod", "uni-config-center"],
      {
        cwd: repoRoot,
        encoding: "utf8",
        stdio: ["ignore", "pipe", "pipe"],
    ...
    execFileSync(
      cliPath,
      ["cloud", "functions", "--upload", "common", "--prj", projectName, "--provider", "aliyun", "--name", "uni-id-common", "--unimod", "uni-id-common"],
      {
        cwd: repoRoot,
        encoding: "utf8",
        stdio: ["ignore", "pipe", "pipe"],
  ...
  const publishOutput = execFileSync(
    cliPath,
    ["publish", "web", "--project", mobileProjectRoot, "--webTitle", "Magazine Digest H0.5"],
    {
      cwd: repoRoot,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
  ...
  fs.writeFileSync(logPath, `${uploadOutput}\n${publishOutput}`, "utf8");
  ...
  try {
    ...
    const requestedPort = Number(process.env.H0_5_H5_PORT || 18080);
    ...
    try {
      const playwrightEnv = {
        ...
        H0_5_TEST_USERNAME: process.env.H0_5_TEST_USERNAME || defaultUsername,
        H0_5_TEST_PASSWORD: process.env.H0_5_TEST_PASSWORD || defaultPassword,
    ...
    writeBlockedReport(error.message || "H0_5_H5_SMOKE_FAILED", {
      hbuilderx_cli_path: cliPath,
      build_dir: buildDir
    });
    process.exit(0);
