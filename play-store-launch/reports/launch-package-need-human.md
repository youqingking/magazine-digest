...
| P0 | 工程/设计 | 补齐 release app 与最终截图规格证据 | 安装 release package 后重跑，或由 owner 明确确认 debug 容器画面可代表最终 app；最终截图完成尺寸、alpha/格式转换和人工公开使用审核。 | 1 |
| P1 | 产品/运营 | 确认上架基础信息 source of truth | 最终 App 名称、备用名、定位、目标用户、账号/删除流程、support URL/email/FAQ 均有 owner 确认。 | 2 |
| P1 | 产品/设计/法务 | 完成截图公开使用审核 | 所有最终商店截图均完成公开使用、内容授权、商标和 claim 审核。 | 3 |
...
| 工程/设计 | 真实截图捕获 | google_play_spec_needs_work | 4 张 PNG 基础规格存在风险：dimension_or_ratio_failed, png_has_alpha, png_not_24_bit_truecolor；affected_shots=shot_01_home_feed, shot_02_detail_deep_read, shot_03_sources_follow, shot_04_profile_summary | 按 Google Play 截图规格转换/裁切，并对最终截图素材重新做尺寸、alpha 和公开使用审核。 |
