import { loadDomainSurfaceManifest } from "./load-domain-surface-manifest.js";

export function mergeDomainSurfaceManifests(manifests = []) {
  const loaded = manifests.map((manifest) => loadDomainSurfaceManifest({ manifest }));
